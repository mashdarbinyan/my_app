...
public class ShoesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(v -> {
            new AlertDialog.Builder(this).setTitle("Add New Shoes").setItems(new String[]{"Take Photo", "Choose from Gallery"}, (dialog, which) -> {
    ...
    private void processAndUpload(Bitmap bitmap) {
        ...
        RequestBody requestBody = new MultipartBody.Builder()
                ...
                .addFormDataPart("image", "shoes.png", RequestBody.create(MediaType.parse("image/png"), bitmapData))
                .build();
        ...
        new OkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                runOnUiThread(() -> Toast.makeText(ShoesActivity.this, "AI Error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
    ...
    private void saveUrlToDatabase(String imageUrl) {
        ...
        FirebaseDatabase.getInstance(dbUrl).getReference("Users").child(userId).child("myShoes").push().setValue(imageUrl);
    }

    private void loadShoes() {
        String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();
        FirebaseDatabase.getInstance(dbUrl).getReference("Users").child(userId).child("myShoes").addValueEventListener(new ValueEventListener() {
            @Override
