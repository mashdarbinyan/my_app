...
public class PantsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(v -> {
            new AlertDialog.Builder(this).setTitle("Add New Pants").setItems(new String[]{"Take Photo", "Choose from Gallery"}, (dialog, which) -> {
    ...
    private void processAndUpload(Bitmap bitmap) {
        ...
        RequestBody requestBody = new MultipartBody.Builder()
                ...
                .addFormDataPart("image", "pants.png", RequestBody.create(MediaType.parse("image/png"), bitmapData))
                .build();
        ...
        new OkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                runOnUiThread(() -> Toast.makeText(PantsActivity.this, "AI Error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
    ...
    private void saveUrlToDatabase(String imageUrl) {
        ...
        FirebaseDatabase.getInstance(dbUrl).getReference("Users").child(userId).child("myPants").push().setValue(imageUrl);
    }

    private void loadPants() {
        String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();
        FirebaseDatabase.getInstance(dbUrl).getReference("Users").child(userId).child("myPants").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                pantsList.clear();
                for (DataSnapshot data : snapshot.getChildren()) {
                    String url = data.getValue(String.class);
                    if (url != null) pantsList.add(url);
                }
